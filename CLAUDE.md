...
- **Connector auth = OAuth.** Claude's custom-connector UI **requires OAuth** (it can't send a header or path secret — those fail with "Couldn't register with the sign-in service"). The server implements a minimal stateless OAuth 2.1 layer (`src/oauth.js` + `api/oauth.js` + `.well-known` rewrites in `vercel.json`): 401-with-`WWW-Authenticate` → protected-resource/auth-server metadata → DCR `/register` → `/authorize` (a **password gate where the password == `MCP_SHARED_SECRET`**) → `/token` (PKCE S256). Codes/tokens are HMAC-signed blobs keyed by `MCP_SHARED_SECRET` (no store). Connector URL is the **plain** `https://<host>/mcp` (not `/mcp/<secret>`). The path-secret/`x-mcp-key`/`?key=` forms still work for curl + Claude Code CLI. The earlier path-secret-only approach did **not** work — the app demands OAuth regardless of URL form.
