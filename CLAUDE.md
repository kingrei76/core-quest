...
- **Art direction:** hand-illustrated 2D — clean line art, soft cel shading, painterly skin gradient, earnest scarred faces, character-portrait quality. Locked May 28, 2026 from Alex's v1 roster sheet at `docs/design/style-bible/alex-v1-roster.png`. The earlier May 4 "HD-2D pixel art (Octopath / Sea of Stars)" lock is superseded; the May 10 CraftPix swap was a workflow shortcut inside the old direction, not a re-pivot. The CraftPix sprites still rendering inside `src/components/spike/EncounterSpike.jsx` are explicitly placeholder pending Alex's in-game character + monster assets.
...
- **Style bible:** `docs/design/style-bible.md` is the durable home for the locked roster reference, palette, typography, and per-archetype delivery briefs for Alex.
- **Design factory:** `design-factory/` was the AI-driven sprite pipeline (PixelLab + Replicate + Aseprite). **Archived May 10, 2026** — see `design-factory/README.md`. Then fully superseded May 28, 2026 by the pivot to commissioned illustration from Alex. Don't add new pipeline work there; don't take its YAMLs as live spec.
